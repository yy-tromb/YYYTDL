...
function getYT(ytURL){
  ...
  if(startsWith(domain,"youtu.be")===true||startsWith(domain,"www.youtube.com")===true||startsWith(domain,"m.youtube.com")===true||startsWith(domain,"music.youtube.com")===true){
   ...
   const ytInitiScript=fixedhtml.split("var ytInitialPlayerResponse = ")[1].split(";</script>")[0];
   ...
   const ytdata=JSON.stringify({"formats":formats,"adaptiveFormats":adaptiveFormats});
  　let output = ContentService.createTextOutput();
  　output.setMimeType(ContentService.MimeType.JSON);
  　output.setContent(ytdata);
...
function err(why){
  reason=why;
  let htmlOutput=HtmlService.createTemplateFromFile("error").evaluate()
